		...
		<div id="app-attachment-upload-container">
			<div id="app-attachment-upload-filelist">
				<ul class="app-attachment-list">
					<script>window.appFileCount = 0;</script>
					<?php
						if ( $attachments ) {
							foreach ( $attachments as $attachment ) {
								echo appthemes_plupload_attach_html( $attachment->ID );
								echo '<script>window.appFileCount += 1;</script>';
							}
						}
					?>
				</ul>
...
function appthemes_plupload_associate_images($post_id, $attachments, $titles = false, $print = false) {
	...
	if ( $count > 0 && $print )
		echo '<p>' . __('Your listing images are now being processed...', APP_TD) . '</p>';
	...
	foreach ( $attachments as $key => $attach_id ) {
		...
		if ( $print )
			echo '<p>' . sprintf( __('Image number %1$d of %2$s has been processed.', APP_TD), $i+1, $count) . '</p>';
...
/**
 * Switch to HTML uploader
 *
 * @return string
 */
function appthemes_plupload_bypass_flash() {
	$link = add_query_arg( array( 'app-plupload' => 'disable' ) );
	echo '<p class="small upload-flash-bypass">';
	printf( __('You are using the flash uploader. Problems? Try the <a href="%s">browser uploader</a> instead.', APP_TD), $link );
	echo '</p>';
}
