...
class CP_Listing_Media extends APP_Media_Manager_Metabox {
	...
	function display( $post ) {
		...
		if ( ! $attachment_ids && ! get_post_meta( $post->ID, '_media_upgraded' ) ) {
			$attachments = get_posts( array( 'post_parent' => $post->ID, 'post_type' => 'attachment', 'nopaging' => true, 'fields' => 'ids' ) );
			...
			update_post_meta( $post->ID, '_media_upgraded', 1 );
		}
